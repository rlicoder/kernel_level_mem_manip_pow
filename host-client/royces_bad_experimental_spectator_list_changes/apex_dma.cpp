    ...
    while(esp_t)
    {
        ...
        while(g_Base!=0 && c_Base!=0)
        {
            ...
            if (esp)
            {
                ...
                apex_mem.Read<uint64_t>(g_Base + OFFSET_LOCAL_ENT, LocalPlayer);
                if (debug)
            		std::cout << "passed localplayer read" << std::endl;
                if (LocalPlayer == 0)
                {
                if (debug)
            		std::cout << "localplayer is zero" << std::endl;
                    next = true;
                ...
                {
                	if( debug)
                		std::cout << "teamplayer check failed: " << team_player << std::endl;
                    next = true;
                    while(next && g_Base!=0 && c_Base!=0 && esp)
                    {
                        std::this_thread::sleep_for(std::chrono::milliseconds(1));
           	 ...
           	 if (debug)
           	 	std::cout << "passed teamplayer check" << std::endl;
                Vector LocalPlayerPosition = LPlayer.getPosition();
                ...
                if (debug)
                	std::cout << "Local player position: " << LocalPlayerPosition.x << " " << LocalPlayerPosition.y << " " << LocalPlayerPosition.z << std::endl;
                ...
                else
                {
                    ...
                    for (int i = 0; i < toRead; i++)
                    {
                        ...
                        {
                            //if (debug)
                                //std::cout << "Centity is 0 or localplayer" << std::endl;
                            continue;
                        }
                        ...
                        {
                            if (debug)
                                std::cout << "Target is not a player or not alive" << std::endl;
                            continue;
                        }
                        ...
                        {
                            if (debug)
                                std::cout << "Your own teammate" << std::endl;
                            continue;
                        }

                        Vector EntityPosition = Target.getPosition();
                        if (debug)
                        	std::cout << "EntityPosition: " << EntityPosition.x << " " << EntityPosition.y << " " << EntityPosition.z << std::endl;
                        float dist = LocalPlayerPosition.DistTo(EntityPosition);
                        if (dist > max_dist || dist < 50.0f)
                        {	
                        	if(debug) {
                        		std::cout << "failed distance check" << std::endl;
                        		std::cout << "dist: " << dist << ", " << max_dist << std::endl;
                        		}
                            continue;
                        }
...
static void SpecList()
{
    while (true)
    {
        ...
        for (int i = 0; i < toRead; i++)
        {
            ...
            char name[33];
            //proto code
            float targetangle = target.GetYaw();
            ...
            apex_mem.Read<uint64_t>(g_Base + OFFSET_LOCAL_ENT, LocalPlayer);
            float localangle;
            apex_mem.Read<float>(LocalPlayer + OFFSET_YAW, localangle);
            ...
            if (localyaw > 360)
                localyaw -= 360;
            if (abs(targetyaw - localyaw) < 10 && !alive) {
                spectatorcount++;
                target.get_name(g_Base, i-1, &speclist[c++].name[0]);
            }
        }
        client_mem.Write<int>(numSpec_addr, cur*10+spectatorcount);
        client_mem.WriteArray<specname>(speclist_addr, speclist, c);
    ...
    while (aim_t)
    {
        ...
        while (g_Base!=0 && c_Base!=0)
        {
            ...
            if (aim>0)
            {
                ...
                {
                    if (debug) {
                        std::cout << "failed  or aiming check in aimbotloop" << std::endl;
                        std::cout << "aimentity: " << aimentity << ", " << "aiming: " << aiming << std::endl;
                    }
                    lock=false;
                    lastaimentity=0;
                    continue;
                ...
                Entity LPlayer = getEntity(LocalPlayer);
                QAngle Angles = CalculateBestBoneAim(LPlayer, aimentity, max_fov, smooth, bone, debug);
